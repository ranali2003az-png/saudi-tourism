...
# NAME / DATA CLEANING
# ------------------------------------------------------------
# Both the local static dataset and the live Places API occasionally
# carry rows with no real venue name (an upstream OSM/Geoapify gap,
# not something we can guess a name for). Rather than show these to
# the user as a literal "Unknown Place" card/stop, they're dropped
# before ranking -- there's nothing useful to recommend without a name.
# ============================================================

_PLACEHOLDER_NAMES = {
    "", "unknown place", "unknown", "unnamed", "unnamed place",
    "n/a", "na", "none", "null", "no name",
}


def _has_real_name(name) -> bool:
    if name is None or (isinstance(name, float) and pd.isna(name)):
        return False
    return str(name).strip().lower() not in _PLACEHOLDER_NAMES


def _drop_unnamed(df: pd.DataFrame, name_col: str = "name") -> pd.DataFrame:
    if df is None or df.empty or name_col not in df.columns:
        return df
    return df[df[name_col].map(_has_real_name)].reset_index(drop=True)


# ============================================================
# CATEGORY-DIVERSE SELECTION
# ------------------------------------------------------------
# A plain "top-N by score" selection tends to get swamped by whichever
# category simply has the most candidates in the dataset (cafes, in
# practice), starving attractions/restaurants out of the final list
# entirely. This guarantees each present category gets a fair share of
# the slots (when the data supports it) before filling any remaining
# slots by score.
# ============================================================

def _select_diverse_top_n(df: pd.DataFrame, score_col: str, top_n: int,
                           type_col: str = "place_type") -> pd.DataFrame:
    if df is None or df.empty:
        return df
    types = [t for t in df[type_col].dropna().unique().tolist() if t]
    if len(types) <= 1 or top_n <= 0:
        return df.sort_values(score_col, ascending=False).head(top_n)

    per_type_quota = max(1, top_n // len(types))
    picked_frames, picked_index = [], set()
    for t in types:
        chunk = df[df[type_col] == t].sort_values(score_col, ascending=False).head(per_type_quota)
        picked_frames.append(chunk)
        picked_index.update(chunk.index)

    picked = pd.concat(picked_frames) if picked_frames else df.head(0)
    remaining = top_n - len(picked)
    if remaining > 0:
        rest = df.drop(index=picked_index).sort_values(score_col, ascending=False).head(remaining)
        picked = pd.concat([picked, rest])
    return picked.sort_values(score_col, ascending=False).head(top_n).reset_index(drop=True)
...
def _prepare_places(rows: list) -> pd.DataFrame:
    ...
    # Data cleaning: never recommend a venue with no real name (see
    # _has_real_name) -- these can't be shown as a meaningful card/stop.
    return _drop_unnamed(out)
...
def recommend_places(city, interests, budget=None, features_df=None, top_n=10,
    ...
    if ml_df is not None and not ml_df.empty:
        ...
        return _select_diverse_top_n(backfilled, "recommendation_score", top_n)

    # --- 2) Fall back to the local static dataset ---
    if features_df is None or city not in LOCAL_DATA_CITIES:
        return pd.DataFrame()

    places = _drop_unnamed(features_df[features_df["city"] == city].copy())
    if excluded_places:
    ...
    return _select_diverse_top_n(places, "recommendation_score", top_n)
...
def baseline_recommend_places(city, features_df, top_n=10):
    ...
    if places.empty:
        return places
    places["_baseline_score"] = places["popularity_score"].fillna(0) * 0.7 + places["distance_score"].fillna(0) * 0.3
    return _select_diverse_top_n(places, "_baseline_score", top_n).drop(columns="_baseline_score")
...
def _nearest_in_pool(pool, cur):
    """Returns (index, distance_km) of the nearest item in pool to cur
    (a free haversine-distance stand-in for a paid routing API), or
    (None, None) if the pool is empty."""
    best_i, best_d = None, float("inf")
    for i, p in enumerate(pool):
        d = _haversine_km(cur[0], cur[1], p.get("latitude"), p.get("longitude"))
        if pd.isna(d):
            d = 999999.0  # unknown location -> deprioritized, not excluded
        if d < best_d:
            best_d, best_i = d, i
    return (best_i, best_d) if best_i is not None else (None, None)


# Order in which categories are offered each scheduling round, and how many
# of each a single day is allowed to accumulate. This is what keeps a day
# from filling up entirely with cafes just because cafes are the most
# numerous category in the data: every round, a sightseeing stop and a
# meal get first refusal before a cafe break is added, and "other" (any
# category outside the three core ones) is only used to fill remaining
# time/budget once the core categories are exhausted or capped.
_CATEGORY_PRIORITY = ["attraction", "restaurant", "cafe"]
_MAX_PER_DAY = {"attraction": 3, "restaurant": 2, "cafe": 2}
...
def generate_itinerary(recommendations, budget, days, hours_per_day, city=None,
    ...
    time budget, daily cost budget, accessibility when required). Stops are
    chosen by rotating through categories (attraction -> restaurant ->
    cafe -> other) each round rather than picking the single nearest place
    overall, so a day gets a mix of a sightseeing stop, a meal, and at
    most a couple of cafe breaks instead of being dominated by whichever
    category has the most candidates. Within a category, the nearest
    unused candidate to the current stop is picked (a free
    nearest-neighbor route, in place of a paid routing API); travel time
    ...
    # --- hard-constraint pre-filter (unchanged) ---
    candidates = []
    ...
    # --- group by category so each day can be scheduled as a mix ---
    by_type = {"attraction": [], "restaurant": [], "cafe": []}
    other = []
    for row in candidates:
        (by_type[row["place_type"]] if row.get("place_type") in by_type else other).append(row)

    daily_budget = budget / days
    center = CITY_CENTERS.get(city, (None, None))

    def _any_left():
        return other or any(by_type.values())

    def _start_cursor():
        if center[0] is not None:
            return center
        for lst in (*by_type.values(), other):
            for p in lst:
                if not pd.isna(p.get("latitude")) and not pd.isna(p.get("longitude")):
    ...
    day_cursor = _start_cursor()

    while _any_left() and day <= days:
        placed_today = []
        cat_counts = {"attraction": 0, "restaurant": 0, "cafe": 0}
        made_progress = True

        while made_progress:
            made_progress = False
            for cat in _CATEGORY_PRIORITY + ["other"]:
                pool = other if cat == "other" else by_type[cat]
                if cat != "other" and cat_counts[cat] >= _MAX_PER_DAY[cat]:
                    continue
                idx, dist = _nearest_in_pool(pool, day_cursor)
                if idx is None:
                    continue
                place = pool[idx]
                km = 0.0 if dist == 999999.0 else dist
                ...
                if day_time + stop_time > hours_per_day or day_cost + stop_cost > daily_budget:
                    continue  # doesn't fit -- leave it for another day, try the next category this round
        ...
        if not placed_today:
            # Nothing left fits today under the hard constraints -- the
            # remaining pool is exhausted or genuinely doesn't fit the
            # time/budget, not something another day would resolve either.
            break
